...
namespace Part1
{
    class Layer
    {
        ...
        public void Draw() {  // Метод Draw: будет рисовать
            ...
            Console.Clear();
            for (int i = Left; i < Math.Min(Content.Length, Right + 1); ++i) { //  будем показывать только 21 контента
                Console.BackgroundColor = (i == SelectedIndex) ? ConsoleColor.Red : ConsoleColor.Black; // при выделений будем красить задний фона на Red
